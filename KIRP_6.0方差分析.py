...
用pandas模块进行存储
'''
with open("Methylation Beta Value4.0.1.txt",'r') as f:
...
if __name__ == '__main__':
    ...
    print "正在读取数据..."
    data_old = pd.read_csv("Methylation Beta Value4.0.1.txt",sep='\t',index_col=0)
    data_t = data_old.T
    ...
    with open("Methylation Beta Value4.0.1.txt",'r') as f:
         for line in f.readlines():
             ...
             if n==rows-1:
                 myList[n]=data1
    print "正在存储数据...."
    filename = "Methylation Beta Value6.0.txt"
...
'''
一共有基因数256，
其中方差齐性假设不成立的有239，这些不满足齐性的基因非参检验具有显着性差异的有233，
满足方差齐性假设的有17，单因素方差分析显示与分期有显著差异的有17
FDR<0.01的条件下有241个位点入选
'''
