...
if __name__ == '__main__':
    ...
    print "正在读取数据。。。。"
    data_new=pd.read_csv("Methylation Beta Value3.4.txt",sep='\t',index_col=0)
    data= data_new.T
    gene= oneway_deal(data)
    print len(gene)
    
    ma=[]
    with open("Methylation Beta Value3.4.txt",'r') as f:
    ...
    filename='Methylation Beta Value3.5.txt'
    write_list_to_file(filename,myList)
    data1=pd.read_csv("Methylation Beta Value3.5.txt",sep='\t',index_col=0)
    genes=list(data1.T)                
    Gene=list(set(genes))
