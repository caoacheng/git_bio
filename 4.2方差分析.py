...
用pandas模块进行存储
'''
with open("Methylation Beta Value4.1.txt",'r') as f:
...
def oneway_deal(data):
    ...
    for xx in range(len(gene_list)):
        ...
        #方差齐性检验
        w,p = stats.levene(*args,center = 'mean')     #方式取'mean'是和spss所做结果一样，默认 'median'        #print "基因%s齐次性检验结果P值为：%.4f" %(xx,p)
        if p<0.05:
...
if __name__ == '__main__':
    ...
    print "正在读取数据..."
    data_old = pd.read_csv("Methylation Beta Value4.1.txt",sep='\t',index_col=0)
    data_t = data_old.T
    ...
    with open("Methylation Beta Value4.1.txt",'r') as f:
         for line in f.readlines():
             ...
             if n==rows-1:
                 myList[n]=data1
    print "正在存储数据...."
    filename = "Methylation Beta Value4.2.txt.txt"
    write_list_to_file(filename,myList)
    data1=pd.read_csv("Methylation Beta Value4.2.txt.txt",sep='\t',index_col=0)
    genes=list(data1.T)                
    Gene=list(set(genes))
