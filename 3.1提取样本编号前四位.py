...
if __name__=='__main__':
    data=pd.read_csv("Methylation Beta Value.txt",sep='\t',index_col=0)
    ...
    data1.to_csv("gene express file3.1.txt",sep='\t',header=True,index=True)
    data.to_csv("Methylation Beta Value3.1.txt",sep='\t',header=True,index=True)
    print "保存完成。。。。"
